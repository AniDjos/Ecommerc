...
$requete2 = "SELECT * FROM livreur where livreur_id = '$code' ";
...
<body data-bs-spy="scroll" data-bs-target=".navbar">
    ...
    <div class="livreur">
        <?php foreach ($detail as $details): ?>
            ...
            <form method="Post" action="validermodiflivreur.php" enctype="multipart/form-data" class="row g-3 needs-validation" novalidate>
                <div class="mb-3" style="margin-right: 20px;">
                    <label for="formFile" class="form-label">Téléphone:</label>
                    <input class="form-control" type="tel" id="formFile" name="phone" value="<?=$details['telephone']?>" required>
                </div>
                <div class="md-4" style="margin-right: 20px;">
                    <label for="validationDefault01" class="form-label">Nom:</label>
                    <input type="text" name="nom" class="form-control" id="validationDefault01" value="<?=$details['nom']?>" required>
                </div>
                <div class="md-4">
                    <label for="validationDefault02" class="form-label">Prenom:</label>
                    <input type="text" name="prenom" class="form-control" id="validationDefault02" value="<?=$details['prenom']?>" required>
                </div>
                <div class="mb-4">
                    <label for="ageInput" class="form-label">Email:</label>
                    <input type="email" name="email" class="form-control" id="ageInput" value="<?=$details['email']?>" required>
                </div>
                <div class="mb-4">
                    <label for="ageInput" class="form-label">Ancien Déplacement:</label>
                    <input type="email" name="email" class="form-control" id="ageInput" value="<?=$details['vehicule']?>" readonly>
                </div>
                <div class="md-4" style="margin-right: 20px;">
                    <label for="validationDefault03" class="form-label"> Nouveau deplacement:</label>
